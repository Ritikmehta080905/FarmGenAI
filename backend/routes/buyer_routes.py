...
@router.get("/offers")
async def get_buyer_offers(user_id: str | None = None, current_user: dict = Depends(get_current_user)):
    return {"offers": get_buyer_offers_controller(user_id=user_id)}


@router.post("/offers")
async def create_buyer_offer(payload: BuyerOfferCreate, current_user: dict = Depends(get_current_user)):
