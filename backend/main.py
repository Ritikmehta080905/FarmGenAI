...
async def on_startup():
    ...
    try:
        ...
        print(f"[WARNING] Redis not reachable ({e}). WebSocket updates will fall back to direct memory updates.")
        redis_client = None
...
app.include_router(negotiation_router, prefix="/api/v1/negotiations", tags=["Negotiations"])  # plural support
...
uploads_dir = os.path.abspath("./node_storage/uploads")
...
async def _handle_ws_connection(websocket: WebSocket, token: str = None):
    if not token:
        token = websocket.query_params.get("token")
    
    from backend.services.security import verify_token
    # If token is provided and not a placeholder/null string, verify it
    if token and token not in ("null", "undefined", "mock_token"):
...
async def ws_all_endpoints(websocket: WebSocket, token: str = None):
    await _handle_ws_connection(websocket, token)
