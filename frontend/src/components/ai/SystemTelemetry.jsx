...
export default function SystemTelemetry() {
    const columns = [
        ...
        { header: 'Current Value', accessor: 'value', className: 'font-mono text-sm' },
        {
            ...
            accessor: 'status',
            render: (row) => (
                <span className={`px-2.5 py-1 text-[10px] uppercase font-bold tracking-wider rounded-md ${row.status === 'Healthy' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700 animate-pulse'
                    }`}>
                    {row.status}
                </span>
            )
