...
export default function AuditLogs() {
    ...
    const columns = [
        ...
        { header: 'Actor / Identity', accessor: 'actor', className: 'font-bold text-slate-800' },
        {
            ...
            accessor: 'action',
            render: (row) => (
                <span className={`px-2 py-1 text-[10px] font-bold tracking-wider rounded uppercase ${row.action.includes('FAILED') || row.action.includes('SUSPEND') ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-700'
                    }`}>
                    {row.action}
                </span>
            )
        ...
        { header: 'Target Resource', accessor: 'resource', className: 'text-sm text-slate-600 font-medium' },
        {
            ...
            accessor: 'status',
            render: (row) => (
                <span className={`flex items-center gap-1.5 text-xs font-bold ${row.status === 'SUCCESS' ? 'text-emerald-600' : 'text-red-600'
                    }`}>
