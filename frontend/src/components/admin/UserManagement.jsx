...
export default function UserManagement() {
    ...
    const columns = [
        ...
        { header: 'Name / Organization', accessor: 'name', className: 'font-bold text-slate-800' },
        {
            ...
            accessor: 'role',
            render: (row) => (
                <span className={`px-2 py-1 text-[10px] font-bold tracking-wider rounded uppercase ${row.role === 'ADMIN' ? 'bg-purple-100 text-purple-700' :
        ...
        },
        {
            ...
            accessor: 'status',
            render: (row) => (
                <span className={`px-2 py-1 text-[10px] font-bold tracking-wider rounded uppercase ${row.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}>
                    {row.status}
                </span>
            )
